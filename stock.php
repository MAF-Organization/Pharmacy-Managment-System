...
$queryf=mysqli_query($conn, "select * from stock") ;
	...
	if($rows>0){
		while($data=mysqli_fetch_array($queryf)){
			...
			$ida=$data['stock_id'];
			if($amnt<50){
				$query_update=mysqli_query($conn, "update stock set status='low' where stock_id=$ida") ;
			}else{
				$query_updates=mysqli_query($conn, "update stock set status='enough' where stock_id=$ida") ;
			}
    ...
    <div class="tabbed_area">  
		...
		*/
        ...
        while($row = mysqli_fetch_array( $result )) {
				...
				<td><a href="delete_stock.php?id=<?php echo $row['stock_id']?>"><img src="https://cdn-icons-png.flaticon.com/512/25/25298.png" width="24" height="24" border="0" /></a></td>
				<?php
		 } 
             echo "</tbody>";
        // close table>
...
?> 
		...
		echo"</select>";?></p>
				...
				<p class="sabm"><input  name="submiti" type="submit" value="Submit" id="submit" style='background-color:black'/></p>
       ...
       $('#costz').val(data);
            ...
            <p class="update"><input type="text" name="newprice" value="" placeholder="New price" required></p>
			   ...
			   if(isset($_POST['updat'])){
				   ...
				   $dn=$_POST['dets'];
				   if($dname=!''){
					   $queryd=mysqli_query($conn,"select * from stock where drug_name='".$dn."'") ;
					   $rowa=mysqli_num_rows($queryd);
					   if($rowa>0){
						   while($dat=mysqli_fetch_array($queryd)){
							   $idt=$dat['stock_id'];
						   }
			   $queryupda=mysqli_query($conn,"update stock set cost='".$newp."' where stock_id='".$idt."'") ;
						   if($queryupda){
       ...
       $('#stockz').val(data);
            ...
            <p class="update"><input type="text" name="newstock" value="" placeholder="Amount to add" required></p>
			   ...
			   if(isset($_POST['add'])){
                   ...
                   $final=$olds+$news;
				   if($dnameh=!''){
					   $queryd=mysqli_query($conn,"select * from stock where drug_name='".$dndg."'") ;
					   $rowa=mysqli_num_rows($queryd);
					   if($rowa>0){
						   while($dat=mysqli_fetch_array($queryd)){
							   $idtd=$dat['stock_id'];
						   }
			   $queryupdua=mysqli_query($conn,"update stock set quantity='".$final."' where stock_id='".$idtd."'") ;
						   if($queryupdua){
							   echo '<script type="text/javascript">'; 
